...
use std::{
    ...
    cmp, env,
    fs::{File, OpenOptions},
    io::{BufRead, BufReader, Error, ErrorKind, Read, Result, Stdin, Stdout, Write, stdin, stdout},
...
impl Drop for RawMode {
    fn drop(&mut self) {
        tcsetattr(STDIN_FILENO, TCSAFLUSH, &self.orig_term).expect("Failed to drop raw mode. OUPS");
    }
}
...
impl Editor {
        ...
        for y in 0..(self.screenrows) {
            ...
            if filerow >= numrows {
                if self.filename.is_none() && self.rows.is_empty() && y == self.screenrows / 3 {
                    let mut msg = "XTRMV editor, ready to suffer ?".to_string();
    ...
    fn try_refresh_screen(&mut self) -> Result<()> {
        self.scroll();
    ...
    fn save_to_file(&mut self) -> Result<usize> {
